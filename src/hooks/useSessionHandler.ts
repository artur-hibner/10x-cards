...
export const useSessionHandler = () => {
  const handleSessionError = useCallback((error: Error, response?: Response) => {
    // Sprawdź czy błąd jest związany z sesją
    if (response?.status === 401 || (error?.message && error.message.includes("Użytkownik nie jest zalogowany"))) {
